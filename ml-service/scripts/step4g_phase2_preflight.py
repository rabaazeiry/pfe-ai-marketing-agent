"""
Step 4g — Phase 2 pre-flight: run Apify Instagram Scraper on 10 Fashion posts
(3 photo + 4 carousel + 3 reel) and validate response shape + downloads.

Carousel-heavy split because Fashion has 503 carousels vs only 70 photos.
"""
import json
import os
import random
...
POSTS_JSON = ROOT / "data" / "step4" / "fashion_posts_list.json"
...
RAW_OUT = META_DIR / "apify_fashion_preflight.json"
RUN_OUT = META_DIR / "apify_fashion_preflight_run.json"
...
sample = (
    ...
    + random.sample(by_type["carousel"], 4)
    + random.sample(by_type["reel"], 3)
)
print(f"Pre-flight sample: {len(sample)} posts (3p+4c+3r — carousel-heavy for Fashion)")
...
print(f"Projected for 842 Fashion  : ${per_post * 842:.2f}")
print(f"Projected runtime (Apify)  : ~{elapsed/len(items)*842/3600:.2f} h "
      f"(based on {elapsed/len(items):.1f} s/post)")

print("\n=== Done — STOP for user approval before launching full Fashion run. ===")
