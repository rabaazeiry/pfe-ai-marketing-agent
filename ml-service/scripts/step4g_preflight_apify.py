"""
Step 4g — Phase 1 pre-flight: run Apify Instagram Post Scraper on 10 Beauty
posts (4 photo + 3 carousel + 3 reel) and validate response shape + downloads.

Outputs:
  data/step4/metadata/apify_beauty_preflight.json   raw Apify dataset
  data/step4/metadata/apify_beauty_preflight_run.json   run metadata (cost)
  data/step4/images/photos/<post_id>.jpg            1 sample photo (resized)
  data/step4/images/carousel_slides/<post_id>_slide_0.jpg   1 sample slide
  data/step4/images/reel_thumbs/<post_id>_thumb.jpg 1 sample reel thumb
  data/step4/videos/<post_id>.mp4                   1 sample reel video
"""
import json
import os
import random
...
POSTS_JSON = ROOT / "data" / "step4" / "beauty_posts_list.json"
...
RAW_OUT = META_DIR / "apify_beauty_preflight.json"
RUN_OUT = META_DIR / "apify_beauty_preflight_run.json"
...
sample = (
    random.sample(by_type["photo"], 4)
    + random.sample(by_type["carousel"], 3)
    + random.sample(by_type["reel"], 3)
)
print(f"Pre-flight sample: {len(sample)} posts (4p+3c+3r)")
...
print(f"Projected for 816 Beauty   : ${per_post * 816:.2f}")
print(f"Projected runtime (Apify)  : ~{elapsed/len(items)*816/3600:.2f} h "
      f"(based on {elapsed/len(items):.1f} s/post)")

print("\n=== Done — STOP for user approval before launching full Beauty run. ===")
