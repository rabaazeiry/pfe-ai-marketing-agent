"""
Step 4g Phase 3 — Full multimodal scrape for HOTELS industry (907 posts).

Pipeline mirrors Phase 1 Beauty / Phase 2 Fashion:
  Step 4 — Apify Instagram Scraper (apify/instagram-scraper) on all 907 URLs
...
POSTS_JSON = DATA / "hotels_posts_list.json"
APIFY_DATASET_OUT = META_DIR / "apify_hotels.json"
APIFY_RUN_OUT = META_DIR / "apify_hotels_run.json"
DOWNLOAD_LOG = META_DIR / "hotels_download_log.json"
FINAL_REPORT = META_DIR / "hotels_phase3_report.json"
...
def step7_report(run_meta, dl_summary, validation, missing_in_apify, total_seconds):
    ...
    report = {
        "phase": "Hotels Phase 3",
        "total_posts_input": 907,
    ...
    print()
    print("=" * 64)
    print("STEP 7 — PHASE 3 HOTELS FINAL REPORT")
    print("=" * 64)
    print(f"Total posts processed   : 907")
    print(f"Apify items returned    : {907 - len(missing_in_apify)}")
...
def main():
    ...
    log(f"Loaded {len(posts)} Hotels posts")
