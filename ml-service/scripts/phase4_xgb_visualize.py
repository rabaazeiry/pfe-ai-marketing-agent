"""Phase 4.2 - Visualizations for the XGBoost model.

Mirror of scripts/phase4_rf_visualize.py for thesis-quality side-by-side
comparison. Produces 5 PNGs in visualizations/v3/:
  1. xgb_shap_beeswarm.png   - SHAP summary (beeswarm) over 200 sampled
                                test rows.
  2. xgb_pred_vs_actual.png  - log1p predicted vs actual, colored by
                                industry, y=x reference, R^2 + rho.
  3. xgb_residuals.png       - log-scale residuals vs predicted; y=0
                                reference + rolling mean.
  4. xgb_gain_vs_shap.png    - Top-15 gain-based importance next to
                                top-15 mean(|SHAP|).
  5. xgb_distributions.png   - Overlapping histograms (original p99
                                capped + log1p full range).

Loads ``models/xgb_best.pkl`` and reconstructs X_test using the SAME
pipeline as scripts/phase4_xgb.py (SEED=42, DROP_COLS=['has_caption',
'views'], NOMINAL_OHE=['content_type','industry_simple','caption_lang'],
stratified on industry_simple). SHAP values cached to
``data/_shap_values_cached_xgb.npz`` (separate cache from RF).
"""
...
# Mirrors phase4_xgb.py / phase4_rf_visualize.py.
# v2 reads xgb_best.pkl + xgb_predictions.parquet, writes xgb_*.png.
# v3 reads xgb_v3.pkl + xgb_v3_predictions.parquet, writes xgb_v3_*.png.
# CRITICAL: v2 and v3 use SEPARATE SHAP cache files because the V3 X_test
# rows differ from V2's (the V3 dataset dropped 40 outliers, shifting the
# stratified split). Sharing the cache would silently apply stale SHAP
# values from the wrong test rows.
# Override from CLI: `python phase4_xgb_visualize.py v2`
...
if DATASET_VERSION == "v2":
    ...
    SHAP_CACHE = ROOT / "data"   / "_shap_values_cached_xgb.npz"
    PNG_PREFIX = "xgb"
    ...
    SHAP_CACHE = ROOT / "data"   / "_shap_values_cached_xgb_v3.npz"
    PNG_PREFIX = "xgb_v3"
    ...
    SHAP_CACHE = ROOT / "data"   / "_shap_values_cached_xgb_v4.npz"
    PNG_PREFIX = "xgb_v4"
...
# --- Style (identical to phase4_rf_visualize.py for cross-model parity) ----
...
def _rebuild_split() -> Tuple[pd.DataFrame, pd.Series, pd.Series, pd.Series]:
    """Replicate phase4_xgb.py's _load_and_prepare + _split exactly."""
...
def _compute_or_load_shap(
    ...
    """Same caching scheme as RF script. Cache key invariants: SEED,
    sample size, column order. Separate cache file from RF."""
...
def plot_shap_beeswarm(X_sub: pd.DataFrame, shap_values: np.ndarray) -> None:
    ...
    fig = plt.gcf()
    fig.suptitle(
        f"SHAP summary (beeswarm) -- XGBoost [{DATASET_VERSION.upper()}]\n"
...
def plot_pred_vs_actual(preds: pd.DataFrame, industry: pd.Series) -> None:
    ...
    ax.set_title(
        f"Predicted vs Actual -- XGBoost [{DATASET_VERSION.upper()}]\n"
...
def plot_residuals(preds: pd.DataFrame) -> None:
    ...
    ax.set_title(
        f"Residuals vs Predicted -- XGBoost [{DATASET_VERSION.upper()}]\n"
...
# --- Plot 4: Gain importance vs SHAP --------------------------------------- #

def plot_gain_vs_shap(
    model, X_sub: pd.DataFrame, shap_values: np.ndarray,
) -> None:
    print("[4/5] Gain importance + SHAP side-by-side ...")
    t0 = time.perf_counter()
    feature_names = list(X_sub.columns)
    # XGBRegressor.feature_importances_ defaults to gain-based importance.
    gain = pd.Series(model.feature_importances_, index=feature_names)
    gain_top = gain.sort_values(ascending=True).tail(15)
    ...
    axL.barh(gain_top.index, gain_top.values, color=MONO_COLOR, edgecolor="white")
    axL.set_title("Top 15 -- Gain-based importance")
    axL.set_xlabel("feature_importances_ (gain)")
    ...
    fig.suptitle(
        f"Feature importance -- Gain vs SHAP (XGBoost [{DATASET_VERSION.upper()}])",
        y=1.0,
    )
    plt.tight_layout(rect=[0, 0.05, 1, 0.97])
    _caption(
        fig,
        "Gain-based importance counts loss reduction at split time; SHAP "
        "marginalizes interactions and is the more reliable global signal.",
    )
    out = OUT_DIR / f"{PNG_PREFIX}_gain_vs_shap.png"
    fig.savefig(out, bbox_inches="tight")
    plt.close(fig)
    print(f"      wrote {out.name}  ({out.stat().st_size/1024:.1f} KB)  "
          f"[{time.perf_counter()-t0:.1f}s]")
...
def plot_distributions(preds: pd.DataFrame) -> None:
    ...
    fig.suptitle(
        f"True vs Predicted distribution -- XGBoost [{DATASET_VERSION.upper()}]",
        y=1.0,
    )
...
def main() -> None:
    ...
    print("=" * 88)
    print(f"phase4_xgb_visualize -- DATASET_VERSION = {DATASET_VERSION!r}")
    print(f"  model:  {MODEL_PATH.name}")
    print(f"  data:   {DATA_PATH.name}")
    print(f"  preds:  {PRED_PATH.name}")
    print(f"  cache:  {SHAP_CACHE.name}")
    ...
    plot_gain_vs_shap(model, X_sub, shap_values)
