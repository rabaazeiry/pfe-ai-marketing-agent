"""
Step 4g Phase 1 — Full multimodal scrape for BEAUTY industry (816 posts).

Pipeline:
  Step 4 — Apify Instagram Scraper (apify/instagram-scraper) on all 816 URLs
...
Idempotent: rerunning will skip already-downloaded files; if the Apify dataset
already exists it is reused unless --rescrape is passed.
"""
import argparse
import json
import os
...
POSTS_JSON = DATA / "beauty_posts_list.json"
APIFY_DATASET_OUT = META_DIR / "apify_beauty.json"
APIFY_RUN_OUT = META_DIR / "apify_beauty_run.json"
DOWNLOAD_LOG = META_DIR / "beauty_download_log.json"
FINAL_REPORT = META_DIR / "beauty_phase1_report.json"
...
def step7_report(run_meta, dl_summary, validation, missing_in_apify, total_seconds):
    ...
    report = {
        "phase": "Beauty Phase 1",
        "total_posts_input": 816,
    ...
    print()
    print("=" * 64)
    print("STEP 7 — PHASE 1 BEAUTY FINAL REPORT")
    print("=" * 64)
    print(f"Total posts processed   : 816")
    print(f"Apify items returned    : {816 - len(missing_in_apify)}")
...
def main():
    ...
    log(f"Loaded {len(posts)} Beauty posts")
