"""
Step 4g — Phase 4 pre-flight: run Apify Instagram Scraper on 10 Restaurants
posts (4 photo + 3 carousel + 3 reel — photo-heavy for Restaurants content mix).
"""
import json
import os
import random
...
POSTS_JSON = ROOT / "data" / "step4" / "restaurants_posts_list.json"
...
RAW_OUT = META_DIR / "apify_restaurants_preflight.json"
RUN_OUT = META_DIR / "apify_restaurants_preflight_run.json"
...
print(f"Pre-flight sample: {len(sample)} posts (4p+3c+3r — photo-heavy for Restaurants)")
...
print(f"Projected for 817 Restaur. : ${per_post * 817:.2f}")
print(f"Projected runtime (Apify)  : ~{elapsed/len(items)*817/3600:.2f} h "
      f"(based on {elapsed/len(items):.1f} s/post)")

print("\n=== Done — STOP for user approval before launching full Restaurants run. ===")
