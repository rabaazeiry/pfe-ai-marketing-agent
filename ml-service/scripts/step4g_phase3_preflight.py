"""
Step 4g — Phase 3 pre-flight: run Apify Instagram Scraper on 10 Hotels posts
(3 photo + 4 carousel + 3 reel — balanced for Hotels content mix).
"""
import json
import os
import random
...
POSTS_JSON = ROOT / "data" / "step4" / "hotels_posts_list.json"
...
RAW_OUT = META_DIR / "apify_hotels_preflight.json"
RUN_OUT = META_DIR / "apify_hotels_preflight_run.json"
...
print(f"Pre-flight sample: {len(sample)} posts (3p+4c+3r — balanced for Hotels)")
...
print(f"Projected for 907 Hotels   : ${per_post * 907:.2f}")
print(f"Projected runtime (Apify)  : ~{elapsed/len(items)*907/3600:.2f} h "
      f"(based on {elapsed/len(items):.1f} s/post)")

print("\n=== Done — STOP for user approval before launching full Hotels run. ===")
