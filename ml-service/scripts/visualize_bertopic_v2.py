"""Phase 3.8 — generate BERTopic v2 (brand-masked) HTML visualizations.

Mirrors scripts/visualize_bertopic.py but points at the v2 model and the
brand-masked corpus. Same six interactive Plotly views written into
ml-service/visualizations/v2/:
...
OUT_DIR = ROOT / "visualizations" / "v2"
INPUT_COLUMN = "caption_masked"
...
def _load_model_and_data() -> Tuple[BERTopic, pd.DataFrame, List[str]]:
    ...
    # Match the v2 training filter chain exactly: has_caption AND non-empty
    # caption_masked (mask can wipe out brand-only posts to "").
    df_in = df[df["has_caption"]].reset_index(drop=True).copy()
    df_in = df_in[df_in[INPUT_COLUMN].str.strip() != ""].reset_index(drop=True)
    print(f"  {len(df_in):,} captioned + non-empty-after-mask posts "
          f"(matches v2 training input).")
    docs = df_in[INPUT_COLUMN].tolist()

    # Re-attach + fit vectorizer on caption_masked docs (per project memory:
    # BERTopic.save() drops the vectorizer config, and topics_per_class /
    # topics_over_time call _c_tf_idf with fit=False which needs a fitted
    # vocabulary). Fitting on the masked corpus produces the same
    # mask-aware vocabulary the model was trained against.
    print("Re-attaching configured vectorizer + fitting on masked corpus ...")
...
def _print_summary(results: List[Tuple[str, Path, str]]) -> None:
    print()
    print("=" * 88)
    print("Phase 3.8 - BERTopic v2 visualizations summary")
    print("=" * 88)
