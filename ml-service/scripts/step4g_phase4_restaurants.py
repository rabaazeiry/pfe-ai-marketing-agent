"""
Step 4g Phase 4 — Full multimodal scrape for RESTAURANTS industry (817 posts).

Pipeline mirrors Phase 1-3:
  Step 4 — Apify Instagram Scraper (apify/instagram-scraper) on all 817 URLs
...
Idempotent. Output dirs are shared with Phase 1+2 — file names are post_id-based
so there is no collision across industries.
"""
import argparse
import json
import os
...
POSTS_JSON = DATA / "restaurants_posts_list.json"
APIFY_DATASET_OUT = META_DIR / "apify_restaurants.json"
APIFY_RUN_OUT = META_DIR / "apify_restaurants_run.json"
DOWNLOAD_LOG = META_DIR / "restaurants_download_log.json"
FINAL_REPORT = META_DIR / "restaurants_phase4_report.json"
...
def step7_report(run_meta, dl_summary, validation, missing_in_apify, total_seconds):
    ...
    report = {
        "phase": "Restaurants Phase 4",
        "total_posts_input": 817,
    ...
    print()
    print("=" * 64)
    print("STEP 7 — PHASE 4 RESTAURANTS FINAL REPORT")
    print("=" * 64)
    print(f"Total posts processed   : 817")
    print(f"Apify items returned    : {817 - len(missing_in_apify)}")
...
def main():
    ...
    log(f"Loaded {len(posts)} Restaurants posts")
