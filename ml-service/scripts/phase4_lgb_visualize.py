"""Phase 4.3 - Visualizations for the LightGBM model.

Mirror of scripts/phase4_xgb_visualize.py for thesis-quality side-by-side
comparison. Produces 5 PNGs in visualizations/v3/:
  1. lgb_shap_beeswarm.png   - SHAP summary (beeswarm) over 200 sampled
                                test rows.
  2. lgb_pred_vs_actual.png  - log1p predicted vs actual, colored by
                                industry, y=x reference, R^2 + rho.
  3. lgb_residuals.png       - log-scale residuals vs predicted; y=0
                                reference + rolling mean.
  4. lgb_gain_vs_shap.png    - Top-15 gain-based importance next to
                                top-15 mean(|SHAP|).
  5. lgb_distributions.png   - Overlapping histograms (original p99
                                capped + log1p full range).

Loads ``models/lgb_v3.pkl`` (or v2 sibling) and reconstructs X_test
using the SAME pipeline as scripts/phase4_lgb.py (SEED=42, DROP_COLS=
['has_caption','views'], NOMINAL_OHE=['content_type','industry_simple',
'caption_lang'], stratified on industry_simple).

SHAP values cached separately per dataset version (v2 / v3 X_test rows
differ after the V3 outlier filter).
"""
...
# Mirrors phase4_xgb_visualize.py.
# v2 reads lgb_best.pkl + lgb_predictions.parquet, writes lgb_*.png.
...
# --- Style (identical to phase4_rf/xgb_visualize.py for cross-model parity)
...
def _rebuild_split() -> Tuple[pd.DataFrame, pd.Series, pd.Series, pd.Series]:
    """Replicate phase4_lgb.py's _load_and_prepare + _split exactly."""
...
def _compute_or_load_shap(
    ...
    """Same caching scheme as RF/XGB scripts. Cache key invariants: SEED,
    sample size, column order. Separate cache file per model + version."""
...
def plot_shap_beeswarm(X_sub: pd.DataFrame, shap_values: np.ndarray) -> None:
    ...
    fig = plt.gcf()
    fig.suptitle(
        f"SHAP summary (beeswarm) -- LightGBM [{DATASET_VERSION.upper()}]\n"
...
def plot_pred_vs_actual(preds: pd.DataFrame, industry: pd.Series) -> None:
    ...
    ax.set_title(
        f"Predicted vs Actual -- LightGBM [{DATASET_VERSION.upper()}]\n"
...
def plot_residuals(preds: pd.DataFrame) -> None:
    ...
    ax.set_title(
        f"Residuals vs Predicted -- LightGBM [{DATASET_VERSION.upper()}]\n"
...
def plot_gain_vs_shap(
    ...
    # LightGBM .feature_importances_ defaults to 'split' counts; gain is
    # the fairer comparator vs XGBoost's gain importance.
    ...
    axL.set_xlabel("booster.feature_importance(gain)")
    ...
    fig.suptitle(
        f"Feature importance -- Gain vs SHAP (LightGBM [{DATASET_VERSION.upper()}])",
        y=1.0,
    )
    plt.tight_layout(rect=[0, 0.05, 1, 0.97])
    _caption(
...
def plot_distributions(preds: pd.DataFrame) -> None:
    ...
    fig.suptitle(
        f"True vs Predicted distribution -- LightGBM [{DATASET_VERSION.upper()}]",
        y=1.0,
    )
...
def main() -> None:
    ...
    print("=" * 88)
    print(f"phase4_lgb_visualize -- DATASET_VERSION = {DATASET_VERSION!r}")
