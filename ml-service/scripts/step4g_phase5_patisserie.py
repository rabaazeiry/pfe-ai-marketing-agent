"""
Step 4g Phase 5 — Full multimodal scrape for PATISSERIE industry (745 posts).

Pipeline mirrors Phase 1-4:
  Step 4 — Apify Instagram Scraper (apify/instagram-scraper) on all 745 URLs
...
POSTS_JSON = DATA / "patisserie_posts_list.json"
APIFY_DATASET_OUT = META_DIR / "apify_patisserie.json"
APIFY_RUN_OUT = META_DIR / "apify_patisserie_run.json"
DOWNLOAD_LOG = META_DIR / "patisserie_download_log.json"
FINAL_REPORT = META_DIR / "patisserie_phase5_report.json"
...
def step7_report(run_meta, dl_summary, validation, missing_in_apify, total_seconds):
    ...
    report = {
        "phase": "Patisserie Phase 5",
        "total_posts_input": 745,
    ...
    print()
    print("=" * 64)
    print("STEP 7 — PHASE 5 PATISSERIE FINAL REPORT")
    print("=" * 64)
    print(f"Total posts processed   : 745")
    print(f"Apify items returned    : {745 - len(missing_in_apify)}")
...
def main():
    ...
    log(f"Loaded {len(posts)} Patisserie posts")
