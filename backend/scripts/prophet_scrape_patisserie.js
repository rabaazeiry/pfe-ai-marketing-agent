/**
 * Prophet Instagram Scraper — PATISSERIE
 * Scrapes posts from 2023-01-01 to today for 8 patisserie brands.
 * Stores raw post data in the `prophet_posts` MongoDB collection.
 * Does NOT touch competitorprofiles.
 *
 * Usage:  node backend/scripts/prophet_scrape_patisserie.js
 *
 * Re-run safe: skips brands that already have posts in prophet_posts.
...
const POLL_INTERVAL  = 10_000;       // 10 s between status polls
const POLL_TIMEOUT   = 60_000;       // 60 s for the poll axios call itself
const MAX_WAIT       = 35 * 60_000;  // 35 min per brand
const BETWEEN_BRANDS = 15_000;       // 15 s gap between brands (avoid concurrency 403)
const FORCE_RESCRAPE = process.argv.includes('--force');

const BRANDS = [
  'patisseriemasmoudi',
  'patisserie_h_by_omar',
  'mamie.karima',
  'lamaisongourmandise',
  'maisonturki',
  'patisserierekik',
...
const prophetPostSchema = new mongoose.Schema({
  ...
  industry       : { type: String, default: 'patisserie' },
...
/** Launch actor run — retry up to 4× on 403 with 30 s back-off */
async function runActor(input, attempt = 1) {
  try {
    ...
    if (status === 403 && attempt <= 4) {
      ...
      console.warn(`     403 on launch (attempt ${attempt}) — waiting ${wait / 1000}s before retry`);
      await sleep(wait);
...
async function scrapeBrand(username, ProphetPost) {
  console.log(`\n  Scraping @${username} ...`);
  ...
  if (!FORCE_RESCRAPE) {
    ...
    if (existing > 0) {
      ...
      return { username, posts: existing, oldest, newest, cost: 0, followers, skipped: true };
    }
  }
  ...
  try {
    ...
    return { username, posts: 0, oldest: null, newest: null, cost: 0, error: err.message };
  }
  ...
  if (run.status !== 'SUCCEEDED') {
    return { username, posts: 0, oldest: null, newest: null, cost, error: `Run status: ${run.status}` };
  }
  ...
  if (items.length === 0) {
    return { username, posts: 0, oldest: null, newest: null, cost, error: 'No items returned' };
  }

  // Followers — check several possible field names
  const followers =
  ...
  if (docs.length === 0) {
    return { username, posts: 0, oldest: null, newest: null, cost, followers, error: 'No posts in date range' };
  }
  ...
  return { username, posts: inserted, oldest, newest, cost, followers };
}
...
function printReport(results) {
  ...
  console.log('  PROPHET SCRAPE — PATISSERIE — FINAL REPORT');
  console.log('═'.repeat(72));
  ...
  console.log('\n' + '─'.repeat(72));
  console.log(`  Total posts patisserie : ${totalPosts}`);
...
async function main() {
  ...
  for (let i = 0; i < BRANDS.length; i++) {
    ...
    // Wait between brands to avoid hitting concurrency limits
